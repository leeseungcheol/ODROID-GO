...
void game_loop() {
  ...
  while (1) {
    ...
    // erase behind pipe
    if (pipes.x <= TFTW) GO.lcd.drawFastVLine(pipes.x+PIPEW, 0, GAMEH, BCKGRDCOL);
...
void game_start() {
  ...
  GO.lcd.fillRect(10, TFTH2 - 20, TFTW-20, 1, TFT_WHITE);
  GO.lcd.fillRect(10, TFTH2 + 32, TFTW-20, 1, TFT_WHITE);
  GO.lcd.setTextColor(TFT_WHITE);
  GO.lcd.setTextSize(3);
  // half width - num char * char width in pixels
  GO.lcd.setCursor( TFTW2-(6*9), TFTH2 - 16);
  GO.lcd.println("FLAPPY");
  GO.lcd.setTextSize(3);
  GO.lcd.setCursor( TFTW2-(6*9), TFTH2 + 8);
  GO.lcd.println("-BIRD-");
  GO.lcd.setTextSize(2);
  GO.lcd.setCursor( 10, TFTH2 - 36);
  GO.lcd.println("ODROID-GO");
  GO.lcd.setCursor( TFTW2 - (17*9), TFTH2 + 36);
  GO.lcd.println("Press the B button");
  while (1) {
...
void game_over() {
  ...
  GO.lcd.print("score: ");
  GO.lcd.print(score);
  GO.lcd.setCursor( TFTW2 - (12*6), TFTH2 + 18);
  GO.lcd.println("press button");
